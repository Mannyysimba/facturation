...
import { COMPANY_INFO, STATUS_LABELS, QUOTE_STATUS_LABELS } from '@/lib/constants';
...
export default function InvoiceView({ id }: { id: string }) {
  ...
  const kind = kindFromNumber(invoice.number);
  const isQuote = kind === 'devis';
  const labelMap = isQuote ? QUOTE_STATUS_LABELS : STATUS_LABELS;
  const docNoun = isQuote ? 'Devis' : 'Facture';
  const backHref = isQuote ? '/?tab=devis' : '/';
  const editHref = isQuote ? `/devis/${invoice.id}/modifier` : `/factures/${invoice.id}/modifier`;

  const ht = totalHT(invoice.lines);
  const vatItems = vatBreakdown(invoice.lines);
  const ttc = totalTTC(invoice.lines);

  async function handleMarkPositive() {
    // encaisse = "Encaissé" pour facture, "Accepté" pour devis (même colonne en base)
    await updateInvoiceStatus(invoice!.id, 'encaisse');
    const updated = await getInvoice(invoice!.id);
    setInvoice(updated || null);
  }

  async function handleMarkRefused() {
    await updateInvoiceStatus(invoice!.id, 'en_retard');
    const updated = await getInvoice(invoice!.id);
    setInvoice(updated || null);
  }
      ...
      <div className="border-b border-zinc-200">
        <div className="max-w-4xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Button variant="outline" onClick={() => router.push(backHref)} className="h-8 text-sm border-zinc-200">
              Retour
            </Button>
            <span className="inline-flex items-center gap-1.5 text-xs text-zinc-600">
              <span className={`w-1.5 h-1.5 rounded-full ${STATUS_DOT[invoice.status]}`} />
              {labelMap[invoice.status]}
            </span>
          </div>
            ...
            {isQuote ? (
              <>
                {invoice.status !== 'encaisse' && (
                  <Button variant="outline" onClick={handleMarkPositive} className="h-8 text-sm border-zinc-200">
                    Marquer accept&eacute;
                  </Button>
                )}
                {invoice.status !== 'en_retard' && (
                  <Button variant="outline" onClick={handleMarkRefused} className="h-8 text-sm border-zinc-200">
      ...
      <div className="max-w-4xl mx-auto px-6 py-8">
        <div className="bg-white border border-zinc-200 rounded-lg p-10 shadow-sm">
          ...
          <div className="mb-8 pb-6 border-b-2 border-slate-900">
            <h1 className="text-2xl font-bold text-slate-900 tracking-tight">{docNoun} {invoice.number}</h1>
            <div className="flex justify-between mt-2">
              <span className="text-sm font-medium text-slate-700">{invoice.title}</span>
              <span className="text-sm text-slate-600">&Eacute;mis{isQuote ? '' : 'e'} le <span className="font-medium text-slate-800">{formatDate(invoice.issueDate)}</span></span>
            </div>
          </div>
          ...
          <div className="flex justify-between mt-6">
            ...
            <div className="text-right">
              <p className="text-[11px] font-semibold uppercase tracking-[0.08em] text-indigo-700">
                {isQuote ? 'Conditions' : 'Mode de paiement'}
              </p>
              {isQuote ? (
                <p className="text-xs text-slate-600 mt-1 leading-relaxed">
                  Bon pour accord à retourner signé<br />
